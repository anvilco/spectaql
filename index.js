...
const loadYaml = require('./app/lib/loadYaml')
...
const defaults = {
  ...
  gruntConfigFile: path.resolve(__dirname, 'app/lib/gruntConfig.js'),
...
const introspectionOptionDefaults = {
  dynamicExamplesProcessingModule: path.resolve(__dirname, './customizations/examples'),
...
function resolvePaths (options, keys = ['targetDir', 'appDir', 'logoFile', 'faviconFile', 'specFile']) {
  keys.forEach((key) => {
    ...
    if (val && val.startsWith('.')) {
      options[key] = path.resolve(__dirname, val)
    }
  })
}
