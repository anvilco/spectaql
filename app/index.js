...
const introspectionOptionDefaults = {
  dynamicExamplesProcessingModule: path.resolve(root, './customizations/examples'),
...
function resolvePaths (options, keys = ['targetDir', 'appDir', 'logoFile', 'faviconFile', 'specFile']) {
  keys.forEach((key) => {
    ...
    if (val && val.startsWith('.')) {
      options[key] = path.resolve(root, val)
    }
  })
}
...
function resolveOptions(options) {
  ...
  if (opts.specFile && opts.specFile.indexOf('test/fixtures') === 0)
    opts.specFile = path.resolve(root, opts.specFile)

  if (opts.appDir && opts.appDir.indexOf('/') !== 0)
    opts.appDir = path.resolve(root, opts.appDir)
